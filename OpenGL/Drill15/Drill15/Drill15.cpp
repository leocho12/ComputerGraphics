...
GLvoid drawScene()
{
	...
	GLuint projectionTransformLoc = glGetUniformLocation(shaderProgramID, "projectionTransform");
	// Surface ¼³Á¤
	if (Surface)
...
void InitBuffer()
{
	...
	glBufferData(GL_ARRAY_BUFFER, sizeof(glm::vec3) * 10, NULL, GL_DYNAMIC_DRAW);
...
void UpdateBuffer(const Shape& shape)
{
	...
	glBindBuffer(GL_ARRAY_BUFFER, vbo[1]);
	glBufferSubData(GL_ARRAY_BUFFER, 0, shape.colors.size() * sizeof(glm::vec3), shape.colors.data());
}
