...
GLvoid drawScene()
{
	...
	if (singleMode)
	{
		...
		// 🔹 현재 도형의 버텍스 개수
		int vertexCount = (shape[i].shapetype == 1) ? 2 :
			(shape[i].shapetype == 2) ? 3 :
			(shape[i].shapetype == 3) ? 4 : 5;

		// 🔹 도형 중심 계산
		float cx = 0.0f, cy = 0.0f;
		for (int v = 0; v < vertexCount; v++) {
			cx += shape[i].shapecoord[v][0];
			cy += shape[i].shapecoord[v][1];
		}
		cx /= vertexCount;
		cy /= vertexCount;

		// 🔹 좌표를 중앙 기준으로 옮기고 확대
		GLfloat tempCoords[5][3];
		for (int v = 0; v < vertexCount; v++) {
			tempCoords[v][0] = (shape[i].shapecoord[v][0] - cx) * 2.0f; // 확대 배율 조절 가능
			tempCoords[v][1] = (shape[i].shapecoord[v][1] - cy) * 2.0f;
			tempCoords[v][2] = 0.0f;
		}

		// 🔹 색상 복사
		GLfloat tempColors[5][3];
		memcpy(tempColors, shape[i].color, sizeof(tempColors));

		// 🔹 버퍼 업데이트 (임시 좌표를 0번 오프셋부터 덮어씀)
		glBindBuffer(GL_ARRAY_BUFFER, vbo[0]);
		glBufferSubData(GL_ARRAY_BUFFER, 0, sizeof(tempCoords), tempCoords);
		glBindBuffer(GL_ARRAY_BUFFER, vbo[1]);
		glBufferSubData(GL_ARRAY_BUFFER, 0, sizeof(tempColors), tempColors);
